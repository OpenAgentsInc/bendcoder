...
int main(void) {
  ...
  check(route_decision("read_code", 0.9, 0.1, 0.1, 0.2, 0.1, 1, "").act == ACT_READ_CODE,
        "a confident read acts");
  check(route_decision("read_code", 0.5, 0.1, 0.1, 0.2, 0.1, 1, "").act == ACT_READ_CODE,
        "a mid-confidence read acts");
  check(route_decision("apply_edit", 0.5, 0.1, 0.1, 0.2, 0.9, 1, "").act == ACT_READ_CODE,
        "the same confidence under the edit floor re-reads");
  check(route_decision("generate_answer", 0.3, 0.9, 0.1, 0.2, 0.9, 1, "").act == ACT_READ_CODE,
        "confidence under the floor re-reads rather than acting");
  check(route_decision("task_complete", 0.3, 0.9, 0.1, 0.2, 0.9, 1, "").act == ACT_TASK_COMPLETE,
        "a decision to stop is not rerouted by the floor");
  check(route_decision("search_code", 0.9, 0.1, 0.9, 0.2, 0.1, 1, "search_code").act == ACT_READ_CODE,
        "repeats high does not take that action again");
  check(route_decision("apply_edit", 0.9, 0.1, 0.1, 1.8, 0.1, 1, "").act == ACT_READ_CODE,
        "top-level risk the goal did not ask for is not run");
  check(route_decision("apply_edit", 0.9, 0.1, 0.1, 1.8, 0.9, 1, "").act == ACT_APPLY_EDIT,
        "top-level risk the goal asked for runs");
  check(route_decision("apply_edit", 0.9, 0.1, 0.1, 0.2, 0.9, 0, "").act == ACT_READ_CODE,
        "an edit before the first read reads first");
  check(route_decision("generate_answer", 0.9, 0.1, 0.1, 0.2, 0.9, 0, "").act == ACT_READ_CODE,
        "an answer on too little information with nothing read reads first");
  check(route_decision("none", 0.9, 0.1, 0.9, 1.8, 0.1, 1, "none").act == ACT_NO_FIT,
        "none passes through to the dispatch halt");
  check(route_decision("bogus", 0.9, 0.1, 0.1, 0.2, 0.1, 1, "").act == ACT_UNRECOGNIZED,
